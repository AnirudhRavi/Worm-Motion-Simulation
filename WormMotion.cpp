...
void head(float max)
{
 ...
 glTranslated(max+3, 0.0, 0.0);//Relative to the maximum of postions of sphere(A, B, C) move 3 units along x-axis
...
void display(void)
{
 ...
 Vector3f dV1=((ellipse_B.position-ellipse_A.position)/l(ellipse_A.position,ellipse_B.position));
 Vector3f dV2=((ellipse_C.position-ellipse_B.position)/l(ellipse_B.position,ellipse_C.position));
 ...
 Vector3f springF_A=dV1*f1;
 Vector3f springF_B=dV1*f1-dV2*f2;
 Vector3f springF_C=dV2*f2;
